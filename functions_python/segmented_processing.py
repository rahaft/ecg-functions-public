                    ...
                    if len(value.shape) == 2:
                        ...
                        if seg_h > seg_w:
                            ...
                            merged_data[key][y_start:y_end, x_start] += (
                                seg_data[:, np.newaxis] * weights[y_start:y_end, x_start:x_start+1]
                            )
                            weight_accumulator[key][y_start:y_end, x_start:x_start+1] += (
                                weights[y_start:y_end, x_start:x_start+1]
                            )
                        else:
                            # Row vector
                            seg_data = value[:seg_x_end]
                            merged_data[key][y_start, x_start:x_end] += (
                                seg_data * weights[y_start:y_start+1, x_start:x_end]
                            )
                            weight_accumulator[key][y_start:y_start+1, x_start:x_end] += (
                                weights[y_start:y_start+1, x_start:x_end]
                            )
