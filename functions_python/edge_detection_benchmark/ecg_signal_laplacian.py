"""
ECG Signal Extraction using Laplacian Edge Detection
...
def process_ecg_signal_laplacian(image_path: str, output_dir: str = None,
                                 apply_skeletonization: bool = True,
                                 apply_geometric_correction: bool = False) -> Dict:
    """
    Process image to extract ECG signal using Laplacian edge detection.
    ...
    laplacian_result = benchmarker.get_laplacian(preprocessed['final'])
    
    # Extract signal coordinates
    signal_result = extractor.digitize_signal(
        laplacian_result['edges'],
    ...
    # Calculate metrics
    metrics = metrics_calc.calculate_all_metrics(laplacian_result['edges'])
    ...
    results = {
        ...
        'metrics': metrics,
        'edge_map': laplacian_result['edges'],
        'skeletonized': signal_result.get('skeletonized'),
        'params': laplacian_result['params']
    }
    ...
    if output_dir:
        ...
        # Save edge map
        edge_path = os.path.join(output_dir, 'ecg_signal_laplacian_edges.png')
        cv2.imwrite(edge_path, laplacian_result['edges'])
        
        # Save skeletonized if available
        if signal_result.get('skeletonized') is not None:
            skeleton_path = os.path.join(output_dir, 'ecg_signal_laplacian_skeleton.png')
        ...
        fig = visualizer.plot_ecg_signal_extraction(
            image,
            laplacian_result['edges'],
            signal_result.get('skeletonized', laplacian_result['edges']),
            coordinates,
            method='Laplacian'
        )
        vis_path = os.path.join(output_dir, 'ecg_signal_laplacian_visualization.png')
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: python ecg_signal_laplacian.py <image_path> [output_dir]")
        sys.exit(1)
    
    image_path = sys.argv[1]
    output_dir = sys.argv[2] if len(sys.argv) > 2 else None
    
    results = process_ecg_signal_laplacian(image_path, output_dir)
    
    print("\n" + "="*60)
    print("ECG SIGNAL EXTRACTION - LAPLACIAN METHOD")
    print("="*60)
