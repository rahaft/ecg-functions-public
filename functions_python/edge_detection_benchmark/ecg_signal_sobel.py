"""
ECG Signal Extraction using Sobel Edge Detection
...
def process_ecg_signal_sobel(image_path: str, output_dir: str = None,
                            apply_skeletonization: bool = True,
                            apply_geometric_correction: bool = False) -> Dict:
    """
    Process image to extract ECG signal using Sobel edge detection.
    ...
    if output_dir:
        ...
        # Save edge map
        edge_path = os.path.join(output_dir, 'ecg_signal_sobel_edges.png')
        cv2.imwrite(edge_path, sobel_result['edges'])
        
        # Save skeletonized if available
        if signal_result.get('skeletonized') is not None:
            skeleton_path = os.path.join(output_dir, 'ecg_signal_sobel_skeleton.png')
        ...
        fig = visualizer.plot_ecg_signal_extraction(
            image,
            sobel_result['edges'],
            signal_result.get('skeletonized', sobel_result['edges']),
            coordinates,
            method='Sobel'
        )
        vis_path = os.path.join(output_dir, 'ecg_signal_sobel_visualization.png')
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: python ecg_signal_sobel.py <image_path> [output_dir]")
        sys.exit(1)
    
    image_path = sys.argv[1]
    output_dir = sys.argv[2] if len(sys.argv) > 2 else None
    
    results = process_ecg_signal_sobel(image_path, output_dir)
    
    print("\n" + "="*60)
    print("ECG SIGNAL EXTRACTION - SOBEL METHOD")
    print("="*60)
