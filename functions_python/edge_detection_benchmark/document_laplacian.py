"""
Document Boundary Detection using Laplacian Edge Detection
...
def process_document_laplacian(image_path: str, output_dir: str = None) -> Dict:
    """
    Process image to extract document boundaries using Laplacian edge detection.
    ...
    doc_result = extractor.find_document(laplacian_result['edges'])
    ...
    if output_dir:
        ...
        # Save edge map
        edge_path = os.path.join(output_dir, 'document_laplacian_edges.png')
        ...
        if doc_result['corners']:
            ...
            vis_path = os.path.join(output_dir, 'document_laplacian_visualization.png')
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: python document_laplacian.py <image_path> [output_dir]")
        sys.exit(1)
    
    image_path = sys.argv[1]
    output_dir = sys.argv[2] if len(sys.argv) > 2 else None
    
    results = process_document_laplacian(image_path, output_dir)
    
    print("\n" + "="*60)
    print("DOCUMENT BOUNDARY DETECTION - LAPLACIAN METHOD")
    print("="*60)
